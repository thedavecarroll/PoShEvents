    ...
    process {
        ...
        <#
            ...
            if ($Event.Id -in 5031,5150,5151,5152,5153,5154,5155,5156,5157,5158,5159) {
                ...
                Add-Member @BaseParams -Name ProtocolDisplayName -Value $ProtocolDisplayName
                Add-Member @BaseParams -Name DirectionDisplayName -Value $DirectionDisplayName
            ...
            if ($Event.ProviderName -eq 'Microsoft-Windows-Security-SPP') {
                if ($Event.Id -eq 12290) {
                    ...
                    Add-Member @BaseParams -Name KMSHost -Value $KmsEventData[2].Trim().Split(':')[0]
                    Add-Member @BaseParams -Name KMSHostPort -Value $KmsEventData[2].Trim().Split(':')[1]
                    Add-Member @BaseParams -Name ClientMachineID -Value $KmsEventData[3].Trim()
                    Add-Member @BaseParams -Name ClientTimestamp -Value (Get-Date $KmsEventData[4].Trim())
                    Add-Member @BaseParams -Name CurrentActivationState -Value (Get-KmsLicenseState -LicenseState $KmsEventData[6].Trim())
                    Add-Member @BaseParams -Name LicenseStateExpirationMin -Value $KmsEventData[7].Trim()
                    Add-Member @BaseParams -Name LicenseStateExpiration -Value (Get-Date $KmsEventData[4].Trim()).AddMinutes($KmsEventData[7].Trim())
                    Add-Member @BaseParams -Name ProductSkuId -Value $KmsEventData[8].Trim()
                    Add-Member @BaseParams -Name ProductSkuName -Value (Get-KmsProductSku -ProductSku $KmsEventData[8].Trim())
                    Add-Member @BaseParams -Name MinActivateCount -Value $KmsEventData[9].Trim()
                    Add-Member @BaseParams -Name KmsErrorCode -Value $KmsEventData[0].Trim()
                    Add-Member @BaseParams -Name KmsErrorMessage -Value (Get-KmsErrorCode -ErrorCode $KmsEventData[0].Trim())
                    ...
                    Add-Member @BaseParams -Name ActivationStatus -Value $Activation
                    Add-Member @BaseParams -Name ClientTimestamp -Value (Get-Date $KmsEventData[7].Trim())
                    Add-Member @BaseParams -Name CurrentActivationCount -Value $KmsEventData[4].Trim()
                    Add-Member @BaseParams -Name NextActivationAttempt -Value (Get-Date $KmsEventData[7].Trim()).AddMinutes($KmsEventData[6].Trim())
                    Add-Member @BaseParams -Name KmsErrorCode -Value $KmsEventData[0].Trim()
                    Add-Member @BaseParams -Name KmsErrorMessage -Value (Get-KmsErrorCode -ErrorCode $KmsEventData[0].Trim())

                } elseif ($Event.Id -eq 12308) {
                    Add-Member @BaseParams -Name ProductSkuId -Value $EventData.Data[0]
                    Add-Member @BaseParams -Name ProductSkuName -Value (Get-KmsProductSku -ProductSku $EventData.Data[0])
                    Add-Member @BaseParams -Name ActivationStatus -Value 'Active Directory Activation has succeeded.'
                    Add-Member @BaseParams -Name ADActivationObjectName -Value $EventData.Data[1]
                    Add-Member @BaseParams -Name ADActivationObject -Value $EventData.Data[2]
            ...
            if ($Reason) {
                ...
                Add-Member @BaseParams -Name LogonMethod -Value $LogonMethod
            }
