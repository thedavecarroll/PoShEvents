...
Properties {
    ...
    $SrcRootDir  = Join-Path -Path $PSScriptRoot -ChildPath $ModuleName
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $ModuleVersion = (Test-ModuleManifest -Path (Join-Path -Path $SrcRootDir -ChildPath "$ModuleName.psd1").Version.ToString())

    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $OutDir = [IO.Path]::Combine($PSScriptRoot,'Release',$ModuleName)

    # The local installation directory for the install task. Defaults to your home Modules location.
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $InstallPath = [IO.Path]::Combine((Split-Path $profile.CurrentUserAllHosts -Parent),'Modules',$ModuleName,$ModuleVersion)
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $ScriptAnalyzerSettingsPath =  Join-Path -Path $PSScriptRoot -ChildPath 'ScriptAnalyzerSettings.psd1'
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $CodeCoverageFiles = (Join-Path -Path $SrcRootDir -ChildPath '*.ps1'), (Join-Path -Path $SrcRootDir -ChildPath '*.psm1')
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $ReleaseNotesPath = Join-Path -Path $PSScriptRoot -ChildPath 'ReleaseNotes.md'
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $SettingsPath = [IO.Path]::Combine($env:LOCALAPPDATA,'Plaster','NewModuleTemplate','SecuredBuildSettings.clixml')
